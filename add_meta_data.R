...
# Read output from 100_most_sim_score.R 
sim_pats <- read_csv("~/Master Thesis/IP_Similarity_Thesis/data_company/most_sim_unique.csv")

# V2: Define input as same name and..
# V3: remove the "problem patent"
# sim_pats <- sim_unique
sim_pats <- sim_pats[-c(450),]


#### 1. Extract Unique patent no. ####
# Add our portfolio & also get meta-information for these
target     <- c(10215337,
                10300636,
                10315342, 
                10392507,
...
portfolio_cpc <- read_csv("~/Master Thesis/IP_Similarity_Thesis/data_company/portfolio_cpc_split3.csv",
                          col_names = T)
portfolio_cpc <-  portfolio_cpc %>%
